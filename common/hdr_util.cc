...
bool ParseVpxCodecPrivate(const uint8_t* private_data, int32_t length,
  ...
  features->profile = -1;
  features->level = -1;
  features->bit_depth = -1;
  features->chroma_subsampling = -1;
  do {
    ...
    if (id_byte == kVp9ProfileId) {
      ...
      if (priv_profile < 0 || priv_profile > 3)
        return false;
      if (features->profile != -1 && features->profile != priv_profile)
        return false;
      ...
      for (int i = 0; i < kNumLevels; ++i) {
        if (priv_level == levels[i]) {
          if (features->level != -1 && features->level != priv_level)
            return false;
      ...
      if (priv_profile != 8 && priv_profile != 10 && priv_profile != 12)
        return false;
      if (features->bit_depth != -1 && features->bit_depth != priv_profile)
        return false;
