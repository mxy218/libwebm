  ...
  if (key_) {
    ...
    if (intra_only_) {
      ...
      for (int i = 0; i < kRefsPerFrame; ++i) {
        VpxReadLiteral(kRefFrames_LOG2);  // Parse ref.
        ReadBit();  // Parse ref sign bias.
      }
...
void Vp9HeaderParser::ParseTileInfo() {
  VpxReadLiteral(2);  // frame context index

  // loopfilter
  VpxReadLiteral(6);  // filter level
  VpxReadLiteral(3);  // sharpness level
  ...
  if (mode_ref_delta_enabled) {
    ...
    if (mode_ref_delta_update) {
      ...
      for (int i = 0; i < kMaxRefLFDeltas; ++i) {
        if (ReadBit())
          VpxReadLiteral(7);  // ref_deltas + sign
      }

      const int kMaxModeDeltas = 2;
      for (int i = 0; i < kMaxModeDeltas; ++i) {
        if (ReadBit())
          VpxReadLiteral(7);  // mode_delta + sign
      }
    }
  }

  // quantization
  VpxReadLiteral(8);  // base_q
  SkipDeltaQ();  // y dc
  SkipDeltaQ();  // uv ac
  SkipDeltaQ();  // uv dc
  ...
  if (!segmentation_enabled) {
    ...
    for (min_log2_n_tiles = 0;
         (kMaxTileWidthB64 << min_log2_n_tiles) < sb_cols;
         min_log2_n_tiles++) {}

    // columns
    int max_log2_tile_cols = max_log2_n_tiles;
    int min_log2_tile_cols = min_log2_n_tiles;
