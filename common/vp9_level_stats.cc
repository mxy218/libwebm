...
void Vp9LevelStats::AddFrame(const Vp9HeaderParser& parser, int64_t time_ns) {
  ...
  while (!luma_window_time_.empty() &&
         luma_window_time_.front() < (time_ns - (kNanosecondsPerSecondi - 1))) {
    current_luma_size_ -= luma_window_size_.front();
    luma_window_size_.pop();
    luma_window_time_.pop();
  }
  luma_window_time_.push(time_ns);
  luma_window_size_.push(luma_picture_size);
  current_luma_size_ += luma_picture_size;
  if (current_luma_size_ > max_luma_size_) {
    max_luma_size_ = current_luma_size_;
    max_luma_end_ns_ = luma_window_time_.back();
  }
  ...
  if (luma_window_time_.size() > max_frames_in_one_second_)
    max_frames_in_one_second_ = luma_window_time_.size();

  // Record CPB stats.
  // Remove all frames that are less than window size.
  while (!cpb_window_time_.empty() && cpb_window_time_.size() > 3)
    cpb_window_time_.pop();
  cpb_window_time_.push(time_ns);

  // Remove all frames that are less than window size.
  while (!window_frame_size_.empty() && window_frame_size_.size() > 3) {
    current_cpb_size_ -= window_frame_size_.front();
    window_frame_size_.pop();
  }
  window_frame_size_.push(parser.frame_size());

  current_cpb_size_ += parser.frame_size();
  if (current_cpb_size_ > max_cpb_size_) {
    max_cpb_size_ = current_cpb_size_;
    max_cpb_start_ns_ = cpb_window_time_.front();
    max_cpb_end_ns_ = cpb_window_time_.back();
  }

  if (max_cpb_window_size_ < static_cast<int64_t>(cpb_window_time_.size())) {
    max_cpb_window_size_ = cpb_window_time_.size();
...
Vp9Level Vp9LevelStats::GetLevel() {
  ...
  // already the the minimum level for all the other requirements.
...
int64_t Vp9LevelStats::GetMaxLumaSampleRate() {
  return max_luma_picture_size_ * max_frames_in_one_second_;
}

int64_t Vp9LevelStats::GetMaxLumaPictureSize() {
  return max_luma_picture_size_;
}

double Vp9LevelStats::GetAverageBitRate() {
...
double Vp9LevelStats::GetMaxCpbSize() { return max_cpb_size_ / 125.0; }

double Vp9LevelStats::GetCompressionRatio() {
  return total_uncompressed_bits_ /
         static_cast<double>(total_compressed_size_ * 8);
}

int Vp9LevelStats::GetMaxColumnTiles() { return max_column_tiles_; }

int Vp9LevelStats::GetMinimumAltrefDistance() {
...
int Vp9LevelStats::GetMaxReferenceFrames() { return max_frames_refreshed_; }

void Vp9LevelStats::SetDuration(int64_t time_ns) { duration_ns_ = time_ns; }
