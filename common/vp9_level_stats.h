...
class Vp9LevelStats {
 public:
  ...
  int64_t GetMaxLumaSampleRate();

  // The extended frame size in pixels (samples).
  int64_t GetMaxLumaPictureSize();
 ...
 private:
  ...
  int64_t max_luma_sample_rate_;

  std::queue<int64_t> luma_window_time_;
  std::queue<int64_t> luma_window_size_;
  ...
  std::queue<int64_t> cpb_window_time_;
  int64_t max_cpb_window_size_;
  int64_t max_cpb_window_end_ns_;

  std::queue<int64_t> window_frame_size_;
