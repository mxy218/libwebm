...
bool BCMVHeader::Write(PacketDataBuffer* buffer) const {
  ...
  const uint8_t bcmv_buffer[kRemainingBytes] = {
      static_cast<std::uint8_t>(((length + size()) >> 24) & 0xff),
      static_cast<std::uint8_t>(((length + size()) >> 16) & 0xff),
      static_cast<std::uint8_t>(((length + size()) >> 8) & 0xff),
      static_cast<std::uint8_t>((length + size()) & 0xff),
      0,
      0 /* 2 bytes 0 padding */};
