...
bool Chapter::AddString(const char* title,
                        const char* language,
                        const char* country) {
  ...
  if (!d.SetLanguage(language))
    return false;

  if (!d.SetCountry(country))
    return false;
...
bool Chapter::Display::SetTitle(const char* title) {
  return StrCpy(title, &title_);
}

bool Chapter::Display::SetLanguage(const char* language) {
  return StrCpy(language, &language_);
}

bool Chapter::Display::SetCountry(const char* country) {
  return StrCpy(country, &country_);
}

uint64 Chapter::Display::WriteDisplay(IMkvWriter* writer) const {
  uint64 payload_size = EbmlElementSize(kMkvChapterString, title_);

  if (language_)
    payload_size += EbmlElementSize(kMkvChapterLanguage, language_);

  if (country_)
    payload_size += EbmlElementSize(kMkvChapterCountry, country_);
  ...
  if (language_) {
    if (!WriteEbmlElement(writer, kMkvChapterLanguage, language_))
      return 0;
  }

  if (country_) {
    if (!WriteEbmlElement(writer, kMkvChapterCountry, country_))
      return 0;
  }
