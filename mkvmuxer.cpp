...
bool Chapters::Write(IMkvWriter* writer) const {
  ...
  // TODO(matthewjheaney): put chapters entry in seekhead

  const uint64 size = WriteEdition(NULL);  // return size only

  if (!WriteEbmlMasterElement(writer, kMkvChapters, size))
    return false;

  if (!WriteEdition(writer))
    return false;
