...
#define BUF_SIZE 2048
  ...
  int64 len = end - start;
  while (len > 0) {
    const int64 read_len = (len > BUF_SIZE) ? BUF_SIZE : len;
...
void Segment::AdjustCuesHelper(uint64 diff, int32 index, uint64* cue_size) {
  CuePoint* cue_point = cues_.GetCueByIndex(index);
  uint64 size = *cue_size;
  uint64 old_cue_point_size = cue_point->Size();
  ...
  //    Let d = a + b + c. Now d is the new size of the Cues element whic is
  //                       passed on to the next recursive call.
  // Using the same variable names in the code as in the comments for clarity.
  uint64 a = *cue_size;
  uint64 b = cue_point->Size() - old_cue_point_size;
  uint64 c = GetCodedUIntSize(a + b) - GetCodedUIntSize(a);
  *cue_size = a + b + c;
  diff = *cue_size - size;
  if (diff > 0) {
    for (int32 i = 0; i < cues_.cue_entries_size(); ++i) {
      AdjustCuesHelper(diff, i, cue_size);  // recursively update the change in size
    }
  }
}

void Segment::AdjustCues() {
  uint64 current_cue_size = cues_.Size();
  uint64 cue_size = current_cue_size;
  for (int32 i = 0; i < cues_.cue_entries_size(); i++)
    AdjustCuesHelper(current_cue_size, i, &cue_size);

  // Adjust the Seek Entry to reflect the change in position
  // of Cluster and Cues
  int32 cluster_id;
  int32 cues_id;
  for (int32 i = 0; i < seek_head_.MaxNumEntries(); ++i) {
