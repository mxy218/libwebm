...
bool ChunkedCopy(mkvmuxer::IMkvReaderWriter* source, mkvmuxer::IMkvWriter* dst,
                 mkvmuxer::int64 start, size_t size) {
// TODO(vigneshv): Check if this is a reasonable value.
#define BUF_SIZE 2048
  uint8 buf[BUF_SIZE];
  int64 offset = start;
  while (size > 0) {
    const int64 read_len = (size > BUF_SIZE) ? BUF_SIZE : size;
...
void Segment::MoveCuesBeforeClustersHelper(uint64 diff,
                                           int32 index,
                                           uint64* cues_size) {
  uint64 old_cues_size = *cues_size;
  ...
  uint64 cue_point_size_diff = cue_point->Size() - old_cue_point_size;
  uint64 cue_size_diff = GetCodedUIntSize(*cues_size + cue_point_size_diff) -
