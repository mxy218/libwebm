...
bool Frame::IsValid() const {
  if (length_ == 0 || frame_ == NULL) {
    return false;
  }
  if ((additional_length_ != 0 && additional_ == NULL) ||
...
bool Frame::IsSimple() const {
  // If one of the following conditions are met, then the Frame cannot be
  // written as a Simple Block:
...
bool Cluster::AddFrameWithAdditional(const uint8* data, uint64 length,
                                     ...
                                     bool is_key) {
  if (additional == NULL || additional_length == 0) {
    return false;
  }
  Frame frame;
  frame.Init(data, length);
...
bool Cluster::DoWriteFrame(const Frame* const frame) {
  ...
  if (!IsValidTrackNumber(frame->track_number()))
    return false;
