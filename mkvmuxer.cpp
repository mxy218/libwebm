...
bool Cluster::AddFrame(const uint8* frame,
  ...
  const char num = static_cast<char>(track_number);
...
bool Cluster::AddMetadata(const uint8* frame,
  ...
  if (rel_block_tc < SHRT_MIN)
    return false;

  if (rel_block_tc > SHRT_MAX)
    return false;

  if (track_number == 0)
    return false;

  if (track_number > 0x7E)
    return false;
...
int Segment::TestFrame(uint64 track_number,
  ...
  // The frame has a timestamp significantly larger than the last
  // cluster, therefore we cannot write this frame that cluster, and
  // so we must create a new cluster.  There is a snag however,
  // because if we have queued (audio) frames, then the new cluster
  // will have a time corresponding to the first queued frame, and so
  // we cannot simply include this frame in that new cluster (because
  // we must compare the time of this frame to that of the new
  // cluster, as we're doing here).  Hence we return 2, instructing
  // the caller to create a new cluster, but in addition loop again,
  // force another comparison of timestamps.
  
  // Note that if there are no queued frames, there's no need ro make
  ...
  // not "too large" then the second time TestFrame is called, it will
  // return 0 and so the frame will get emitted.)
...
bool Segment::NewCluster(uint64 track_number,
  ...
  for (;;) {
    ...
    // We created a new cluster, and write some audio frames to the
