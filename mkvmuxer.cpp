...
// having time frame_timestamp_ns.  There is a separate issue of
// whether this frame should be written to the newly-created cluster,
// or deferred to some cluster we create later.
//
int Segment::TestFrame(uint64 track_number,
  ...
  if (cluster_list_size_ <= 0)
    return 1;  // create a new cluster, and put this frame in the new cluster
  ...
  if (frame_timecode < cluster_timecode)  // should never happen
    return -1;  // error

  // The frame we are testing has a timestamp equal to the cluster's
  // timestamp.  Do not create a new cluster -- instead force this
  // frame to be written to the existing cluster.

  if (frame_timecode == cluster_timecode)
  ...
  // optimization can be done by the caller; see
  // Segment::DoNewClusterProcessing.)

  // Note also that these circumlocuations (here and below) are
  // necessary because we don't consider queued frames here.

  const int64 delta_timecode = frame_timecode - cluster_timecode;
  ...
  if (is_key && tracks_.TrackIsVideo(track_number))
    return 1;  // createa a new cluster, and put this frame in new cluster
...
bool Segment::DoNewClusterProcessing(uint64 track_number,
  ...
  for (;;) {
    ...
    // If TestFrame returned 2, we have created a new cluster, but we
    // must perform another test, so we can't simply emit the current
    // frame immediately (because we might have to create yet another
    // new cluster).  We need to re-test because emitting the queued
    // audio frames (in WriteFramesAll) creates a cluster having
    // characteristics unknown at the time of the original test (when
    // TestFrame was called). However, if there we no extant audio
    // frames (WriteFramesAll returned a 0 count), another test will
    // simply return 0 (meaning: "emit current frame immediately, in
    // current cluster"), so we optimize-away the test by returning
    // when the frame count is 0.

    if (frame_count <= 0)  // emit this frame now
      return true;
