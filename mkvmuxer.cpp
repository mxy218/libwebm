...
uint64 ContentEncoding::Size() const {
  ...
  uint64 encoding_size = EbmlElementSize(kMkvContentEncryption,
                                     encryption_size,
                                     true) +
                         encryption_size;
  ...
  encoding_size += EbmlElementSize(kMkvContentEncodingOrder,
                                   encoding_order_,
                                   false);
  uint64 encodings_size = EbmlElementSize(kMkvContentEncoding,
                                          encoding_size,
                                          true) +
                          encoding_size;
...
bool ContentEncoding::Write(IMkvWriter* writer) const {
  ...
  encoding_size += EbmlElementSize(kMkvContentEncodingOrder,
                                   encoding_order_,
                                   false);
  uint64 size = EbmlElementSize(kMkvContentEncoding, encoding_size, true) +
                encoding_size;
...
bool Track::AddContentEncoding() {
  ...
  ContentEncoding* content_encoding = new (std::nothrow) ContentEncoding();
  if (!content_encoding)
    return false;
...
ContentEncoding* const Track::GetContentEncodingByIndex(uint32 index) const {
