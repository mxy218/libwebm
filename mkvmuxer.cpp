...
bool Frame::Init(const uint8* frame, uint64 length) {
  uint8* const data = new (std::nothrow) uint8[static_cast<size_t>(length)]; // NOLINT
  if (!data)
    return false;
...
bool Cues::AddCue(CuePoint* cue) {
  ...
  if ((cue_entries_size_ + 1) > cue_entries_capacity_) {
    ...
    CuePoint** const cues = new (std::nothrow) CuePoint*[new_capacity]; // NOLINT
    if (!cues)
      return false;
...
bool ContentEncoding::SetEncryptionID(const uint8* id, uint64 length) {
  ...
  enc_key_id_ = new (std::nothrow) uint8[static_cast<size_t>(length)]; // NOLINT
  if (!enc_key_id_)
    return false;
