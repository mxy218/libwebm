...
bool WriteEbmlHeader(IMkvWriter* writer, uint64_t doc_type_version) {
  // Level 0
  uint64_t size = EbmlElementSize(kMkvEBMLVersion, 1ULL);
...
void Chapter::set_time(const Segment& segment, uint64_t start_ns, uint64_t end_ns) {
...
uint64_t Chapter::WriteAtom(IMkvWriter* writer) const {
  uint64_t payload_size = EbmlElementSize(kMkvChapterStringUID, id_) +
...
bool Cluster::AddFrame(const uint8_t* data, uint64_t length, uint64_t track_number,
...
bool Cluster::AddMetadata(const uint8_t* data, uint64_t length, uint64_t track_number,
                          uint64_t abs_timecode, uint64_t duration_timecode) {
  Frame frame;
  if (!frame.Init(data, length))
    return false;
  frame.set_track_number(track_number);
  ...
  if (writer->Seekable()) {
    ...
    for (int32_t i = 0; i < kSeekEntryCount; ++i) {
      if (seek_entry_id_[i] != 0) {
        entry_size[i] =
            EbmlElementSize(kMkvSeekID, static_cast<uint64_t>(seek_entry_id_[i]));
...
uint64_t Segment::AddAudioTrack(int32_t sample_rate, int32_t channels, int32_t number) {
...
bool Segment::AddFrame(const uint8_t* data, uint64_t length, uint64_t track_number,
...
bool Segment::AddMetadata(const uint8_t* data, uint64_t length, uint64_t track_number,
