...
class Frame {
 public:
  ...
  int64_t reference_block_timestamp() const { return reference_block_timestamp_; }
...
class ContentEncoding {
 ...
 private:
  ...
  uint64_t EncodingSize(uint64_t compresion_size, uint64_t encryption_size) const;
...
class Cluster {
 public:
  ...
  bool AddFrameWithDiscardPadding(const uint8_t* data, uint64_t length,
                                  int64_t discard_padding, uint64_t track_number,
                                  uint64_t abs_timecode, bool is_key);
...
class Segment {
 ...
 private:
  ...
  bool DoNewClusterProcessing(uint64_t track_num, uint64_t timestamp_ns, bool key);
  ...
  void MoveCuesBeforeClustersHelper(uint64_t diff, int index, uint64_t* cue_size);
