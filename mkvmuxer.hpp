...
class Chapter {
 ...
 public:
  ...
  void SetTime(const Segment& segment,
               uint64 start_time_ns,
               uint64 end_time_ns);

  // Add an English-language chapter title to this chapter.
  // TODO(matthewjheaney): add support for non-English languages
  bool AddTitle(const char* title);
 ...
 private:
  ...
  // Copy the |src| string to |dst|.  Return true on success.
  static bool StrCpy(const char* src, char** dst);
  ...
  struct Display {
    ...
    char* title_;
    // TODO(matthewjheaney): add language and country indictors
  };
