...
class Frame {
 ...
 private:
  ...
  // will cause BlockDuration to be written (thereby making the frame written in
  // a Block) when added to the Cluster.
  bool duration_set_;
...
class Cluster {
 public:
  ...
  // variant of Finalize() must not be called when
  // |write_last_frame_with_duration_| is set to true.
  bool Finalize();
 ...
 private:
  ...
  // of frames and the parametrized version of Finalize() must be called to
