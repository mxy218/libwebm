...
bool VideoTrack::Write(IMkvWriter* writer) const {
  ...
  if (pixel_width_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelWidth,
                          static_cast<uint64>(pixel_width_)))
      return false;
  } else {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelWidth,
  ...
  if (pixel_height_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelHeight,
                          static_cast<uint64>(pixel_height_)))
      return false;
  } else {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelHeight,
...
uint64_t VideoTrack::VideoPayloadSize() const {
  uint64_t size =
      EbmlElementSize(libwebm::kMkvPixelWidth,
                      static_cast<uint64>((pixel_width_ > 0) ? pixel_width_ :
                                          width_));
  size +=
      EbmlElementSize(libwebm::kMkvPixelHeight,
                      static_cast<uint64>((pixel_height_ > 0 ) ? pixel_height_ :
                                          height_));
  if (display_width_ > 0)
