...
bool Projection::Write(IMkvWriter* writer) const {
  ...
  if (private_data_length > 0 && private_data != NULL &&
      !WriteEbmlElement(writer, libwebm::kMkvProjectionPrivate, private_data,
                        private_data_length)) {
    return false;
  }
...
bool Projection::SetProjectionPrivate(const uint8_t* data,
  ...
  private_data_length = data_length;
  memcpy(private_data, data, static_cast<size_t>(data_length));
...
uint64_t Projection::PayloadSize() const {
  ...
  if (private_data_length > 0 && private_data != NULL) {
    size += EbmlElementSize(libwebm::kMkvProjectionPrivate, private_data,
                            private_data_length);
  }
...
bool VideoTrack::SetProjection(const Projection& projection) {
  ...
  if (projection.private_data) {
    if (!projection_ptr->SetProjectionPrivate(projection.private_data,
                                              projection.private_data_length)) {
      return false;
    }
  }
