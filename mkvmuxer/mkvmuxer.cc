...
bool WriteEbmlHeader(IMkvWriter* writer, uint64_t doc_type_version) {
  // Level 0
  uint64 size = EbmlElementSize(libwebm::kMkvEBMLVersion,
                                static_cast<uint64>(UINT64_C(1)));
  size += EbmlElementSize(libwebm::kMkvEBMLReadVersion,
                          static_cast<uint64>(UINT64_C(1)));
  size += EbmlElementSize(libwebm::kMkvEBMLMaxIDLength,
                          static_cast<uint64>(UINT64_C(4)));
  size += EbmlElementSize(libwebm::kMkvEBMLMaxSizeLength,
                          static_cast<uint64>(UINT64_C(8)));
  ...
  size += EbmlElementSize(libwebm::kMkvDocTypeReadVersion,
                          static_cast<uint64>(UINT64_C(2)));
...
bool CuePoint::Write(IMkvWriter* writer) const {
  ...
  uint64 size = EbmlElementSize(libwebm::kMkvCueClusterPosition,
...
uint64_t ContentEncAESSettings::PayloadSize() const {
  uint64_t size =
      EbmlElementSize(libwebm::kMkvAESSettingsCipherMode,
                      static_cast<uint64>(cipher_mode_));
  return size;
}
...
uint64_t Track::PayloadSize() const {
  uint64_t size = EbmlElementSize(libwebm::kMkvTrackNumber,
                                  static_cast<uint64>(number_));
  size += EbmlElementSize(libwebm::kMkvTrackUID,
                          static_cast<uint64>(uid_));
  size += EbmlElementSize(libwebm::kMkvTrackType,
...
bool Track::Write(IMkvWriter* writer) const {
  ...
  if (max_block_additional_id_)
    size += EbmlElementSize(libwebm::kMkvMaxBlockAdditionID,
                            static_cast<uint64>(max_block_additional_id_));
  if (codec_delay_)
    size += EbmlElementSize(libwebm::kMkvCodecDelay, static_cast<uint64>(codec_delay_));
  if (seek_pre_roll_)
    size += EbmlElementSize(libwebm::kMkvSeekPreRoll, static_cast<uint64>(seek_pre_roll_));
  if (default_duration_)
    size += EbmlElementSize(libwebm::kMkvDefaultDuration, static_cast<uint64>(default_duration_));

  const int64_t payload_position = writer->Position();
  if (payload_position < 0)
    return false;

  if (!WriteEbmlElement(writer, libwebm::kMkvTrackNumber, static_cast<uint64>(number_)))
    return false;
  if (!WriteEbmlElement(writer, libwebm::kMkvTrackUID, static_cast<uint64>(uid_)))
    return false;
  if (!WriteEbmlElement(writer, libwebm::kMkvTrackType, static_cast<uint64>(type_)))
    return false;
  if (max_block_additional_id_) {
  ...
  if (codec_delay_) {
    if (!WriteEbmlElement(writer, libwebm::kMkvCodecDelay, static_cast<uint64>(codec_delay_)))
      return false;
  }
  if (seek_pre_roll_) {
    if (!WriteEbmlElement(writer, libwebm::kMkvSeekPreRoll, static_cast<uint64>(seek_pre_roll_)))
      return false;
  }
  if (default_duration_) {
...
bool Colour::Write(IMkvWriter* writer) const {
  ...
  if (range != kValueNotPresent &&
      !WriteEbmlElement(writer, libwebm::kMkvRange, static_cast<uint64>(range))) {
    return false;
  }
  ...
  if (primaries != kValueNotPresent &&
      !WriteEbmlElement(writer, libwebm::kMkvPrimaries, static_cast<uint64>(primaries))) {
    return false;
  }
  if (max_cll != kValueNotPresent &&
      !WriteEbmlElement(writer, libwebm::kMkvMaxCLL, static_cast<uint64>(max_cll))) {
    return false;
  }
  if (max_fall != kValueNotPresent &&
      !WriteEbmlElement(writer, libwebm::kMkvMaxFALL,static_cast<uint64>(max_fall))) {
    return false;
  }
...
uint64_t Colour::PayloadSize() const {
  ...
  if (matrix_coefficients != kValueNotPresent)
    size +=
        EbmlElementSize(libwebm::kMkvMatrixCoefficients, static_cast<uint64>(matrix_coefficients));
  if (bits_per_channel != kValueNotPresent)
    size += EbmlElementSize(libwebm::kMkvBitsPerChannel, static_cast<uint64>(bits_per_channel));
  if (chroma_subsampling_horz != kValueNotPresent)
  ...
  if (cb_subsampling_horz != kValueNotPresent)
    size +=
        EbmlElementSize(libwebm::kMkvCbSubsamplingHorz, static_cast<uint64>(cb_subsampling_horz));
  if (cb_subsampling_vert != kValueNotPresent)
    size +=
        EbmlElementSize(libwebm::kMkvCbSubsamplingVert, static_cast<uint64>(cb_subsampling_vert));
  if (chroma_siting_horz != kValueNotPresent)
    size += EbmlElementSize(libwebm::kMkvChromaSitingHorz, static_cast<uint64>(chroma_siting_horz));
  if (chroma_siting_vert != kValueNotPresent)
    size += EbmlElementSize(libwebm::kMkvChromaSitingVert, static_cast<uint64>(chroma_siting_vert));
  if (range != kValueNotPresent)
  ...
  if (transfer_characteristics != kValueNotPresent)
    size += EbmlElementSize(libwebm::kMkvTransferCharacteristics,
                            static_cast<uint64>(transfer_characteristics));
  if (primaries != kValueNotPresent)
    size += EbmlElementSize(libwebm::kMkvPrimaries, static_cast<uint64>(primaries));
  if (max_cll != kValueNotPresent)
    size += EbmlElementSize(libwebm::kMkvMaxCLL, static_cast<uint64>(max_cll));
  if (max_fall != kValueNotPresent)
    size += EbmlElementSize(libwebm::kMkvMaxFALL, static_cast<uint64>(max_fall));
...
bool VideoTrack::Write(IMkvWriter* writer) const {
  ...
  if (!WriteEbmlElement(writer, libwebm::kMkvPixelWidth, static_cast<uint64>(width_)))
    return false;
  if (!WriteEbmlElement(writer, libwebm::kMkvPixelHeight, static_cast<uint64>(height_)))
    return false;
  if (display_width_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvDisplayWidth, static_cast<uint64>(display_width_)))
      return false;
  }
  if (display_height_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvDisplayHeight, static_cast<uint64>(display_height_)))
      return false;
  }
  if (crop_left_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelCropLeft, static_cast<uint64>(crop_left_)))
      return false;
  }
  if (crop_right_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelCropRight, static_cast<uint64>(crop_right_)))
      return false;
  }
  if (crop_top_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelCropTop, static_cast<uint64>(crop_top_)))
      return false;
  }
  if (crop_bottom_ > 0) {
    if (!WriteEbmlElement(writer, libwebm::kMkvPixelCropBottom, static_cast<uint64>(crop_bottom_)))
      return false;
  }
  if (stereo_mode_ > kMono) {
    if (!WriteEbmlElement(writer, libwebm::kMkvStereoMode, static_cast<uint64>(stereo_mode_)))
      return false;
  }
  if (alpha_mode_ > kNoAlpha) {
    if (!WriteEbmlElement(writer, libwebm::kMkvAlphaMode, static_cast<uint64>(alpha_mode_)))
      return false;
  }
  if (frame_rate_ > 0.0) {
...
uint64_t VideoTrack::VideoPayloadSize() const {
  uint64_t size = EbmlElementSize(libwebm::kMkvPixelWidth, static_cast<uint64>(width_));
  size += EbmlElementSize(libwebm::kMkvPixelHeight, static_cast<uint64>(height_));
  if (display_width_ > 0)
    size += EbmlElementSize(libwebm::kMkvDisplayWidth, static_cast<uint64>(display_width_));
  if (display_height_ > 0)
    size += EbmlElementSize(libwebm::kMkvDisplayHeight, static_cast<uint64>(display_height_));
  if (crop_left_ > 0)
    size += EbmlElementSize(libwebm::kMkvPixelCropLeft, static_cast<uint64>(crop_left_));
  if (crop_right_ > 0)
    size += EbmlElementSize(libwebm::kMkvPixelCropRight, static_cast<uint64>(crop_right_));
  if (crop_top_ > 0)
    size += EbmlElementSize(libwebm::kMkvPixelCropTop, static_cast<uint64>(crop_top_));
  if (crop_bottom_ > 0)
    size += EbmlElementSize(libwebm::kMkvPixelCropBottom, static_cast<uint64>(crop_bottom_));
  if (stereo_mode_ > kMono)
    size += EbmlElementSize(libwebm::kMkvStereoMode, static_cast<uint64>(stereo_mode_));
  if (alpha_mode_ > kNoAlpha)
    size += EbmlElementSize(libwebm::kMkvAlphaMode, static_cast<uint64>(alpha_mode_));
  if (frame_rate_ > 0.0)
...
uint64_t AudioTrack::PayloadSize() const {
  ...
  size += EbmlElementSize(libwebm::kMkvChannels, static_cast<uint64>(channels_));
  if (bit_depth_ > 0)
    size += EbmlElementSize(libwebm::kMkvBitDepth, static_cast<uint64>(bit_depth_));
...
bool AudioTrack::Write(IMkvWriter* writer) const {
  ...
  if (!WriteEbmlElement(writer, libwebm::kMkvSamplingFrequency,
                        static_cast<float>(sample_rate_)))
    return false;
  if (!WriteEbmlElement(writer, libwebm::kMkvChannels, static_cast<uint64>(channels_)))
    return false;
  if (bit_depth_ > 0)
    if (!WriteEbmlElement(writer, libwebm::kMkvBitDepth, static_cast<uint64>(bit_depth_)))
      return false;
...
uint64_t Chapter::WriteAtom(IMkvWriter* writer) const {
  uint64 payload_size =
      ...
      EbmlElementSize(libwebm::kMkvChapterTimeStart, static_cast<uint64>(start_timecode_)) +
      EbmlElementSize(libwebm::kMkvChapterTimeEnd, static_cast<uint64>(end_timecode_));
  ...
  if (!WriteEbmlElement(writer, libwebm::kMkvChapterUID, static_cast<uint64>(uid_)))
    return 0;

  if (!WriteEbmlElement(writer, libwebm::kMkvChapterTimeStart, static_cast<uint64>(start_timecode_)))
    return 0;

  if (!WriteEbmlElement(writer, libwebm::kMkvChapterTimeEnd, static_cast<uint64>(end_timecode_)))
    return 0;
    ...
    for (int32_t i = 0; i < kSeekEntryCount; ++i) {
      if (seek_entry_id_[i] != 0) {
        entry_size[i] = EbmlElementSize(
            libwebm::kMkvSeekID, static_cast<uint64>(seek_entry_id_[i]));
        entry_size[i] +=
            EbmlElementSize(libwebm::kMkvSeekPosition, static_cast<uint64>(seek_entry_pos_[i]));
...
uint64_t SeekHead::MaxEntrySize() const {
  const uint64 max_entry_payload_size =
      EbmlElementSize(libwebm::kMkvSeekID, static_cast<uint64>(UINT64_C(0xffffffff))) +
      EbmlElementSize(libwebm::kMkvSeekPosition, static_cast<uint64>(UINT64_C(0xffffffffffffffff)));
...
bool SegmentInfo::Write(IMkvWriter* writer) {
  ...
  uint64_t size = EbmlElementSize(libwebm::kMkvTimecodeScale, static_cast<uint64>(timecode_scale_));
  if (duration_ > 0.0)
    size +=
  ...
  if (!WriteEbmlElement(writer, libwebm::kMkvTimecodeScale, static_cast<uint64>(timecode_scale_)))
    return false;
