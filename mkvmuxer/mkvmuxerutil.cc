...
uint64 WriteBlock(IMkvWriter* writer, const Frame* const frame, int64 timecode,
  ...
  if (frame->additional()) {
    ...
    block_addid_elem_size =
        EbmlElementSize(libwebm::kMkvBlockAddID, static_cast<uint64>(frame->add_id()));
  ...
  if (frame->discard_padding() != 0) {
    discard_padding_elem_size =
        EbmlElementSize(libwebm::kMkvDiscardPadding, static_cast<int64>(frame->discard_padding()));
  }
  ...
  if (frame->additional()) {
    ...
    if (!WriteEbmlElement(writer, libwebm::kMkvBlockAddID, static_cast<uint64>(frame->add_id())))
      return 0;
