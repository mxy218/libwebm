...
uint64_t WriteBlock(IMkvWriter* writer, const Frame* const frame,
  ...
  if (frame->additional()) {
    ...
    block_addid_elem_size = EbmlElementSize(libwebm::kMkvBlockAddID, frame->add_id());
  ...
  if (duration > 0)
    block_duration_elem_size = EbmlElementSize(libwebm::kMkvBlockDuration, duration);

  const uint64_t block_payload_size = 4 + frame->length();
  const uint64_t block_elem_size =
      EbmlMasterElementSize(libwebm::kMkvBlock, block_payload_size) + block_payload_size;
  ...
  if (frame->additional()) {
    ...
    if (!WriteEbmlMasterElement(writer, libwebm::kMkvBlockMore, block_more_payload_size))
      return 0;

    if (!WriteEbmlElement(writer, libwebm::kMkvBlockAddID, frame->add_id()))
      return 0;

    if (!WriteEbmlElement(writer, libwebm::kMkvBlockAdditional, frame->additional(),
  ...
  if (frame->discard_padding() != 0 &&
      !WriteEbmlElement(writer, libwebm::kMkvDiscardPadding, frame->discard_padding())) {
    return false;
  }
  ...
  if (duration > 0 && !WriteEbmlElement(writer, libwebm::kMkvBlockDuration, duration)) {
    return false;
  }
  return EbmlMasterElementSize(libwebm::kMkvBlockGroup, block_group_payload_size) +
         block_group_payload_size;
}
