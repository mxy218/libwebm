...
void Usage() {
  ...
  printf("                                   2: Cube map\n");
  printf("                                   3: Mesh\n");
  printf("  -projection_private_file <int> Override projection private data with\n");
  printf("                                 contents of this file\n");
...
int main(int argc, char* argv[]) {
  ...
  const char* projection_private_file = 0;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (!strcmp("-projection_private_file", argv[i]) && i < argc_check) {
      projection_private_file = argv[++i];
      ...
      if (pVideoTrack->GetProjection() ||
        ...
        if (projection_type != mkvparser::Projection::kTypeNotPresent) {
          ...
          if (projection_type == mkvparser::Projection::kRectangular &&
              projection_private_file != nullptr) {
          ...
          if (projection_private_file != nullptr) {
            std::string contents;
            if (!libwebm::GetFileContents(projection_private_file, &contents) ||
                contents.size() == 0) {
              printf("\n Failed to read file \"%s\" or file is empty\n",
                     projection_private_file);
              return EXIT_FAILURE;
            }
            if (!muxer_projection.SetProjectionPrivate(
                    reinterpret_cast<uint8_t*>(&contents[0]), contents.size())) {
              printf("\n Failed to SetProjectionPrivate of length %lu.\n",
                     contents.size());
              return EXIT_FAILURE;
            }
          }
