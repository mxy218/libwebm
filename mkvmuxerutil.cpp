...
uint64 WriteFrame(IMkvWriter* writer, const Frame* const frame,
  ...
  return frame->IsSimple() ?
      WriteSimpleBlock(writer, frame, timestamp) :
      WriteBlock(writer, frame, timestamp,
                 frame->duration() / cluster->timecode_scale());
}

uint64 WriteBlock(IMkvWriter* writer, const Frame* const frame,
                  int64 timestamp, uint64 duration) {
...
uint64 WriteSimpleBlock(IMkvWriter* writer, const Frame* const frame,
  ...
  return GetUIntSize(kMkvSimpleBlock) + GetCodedUIntSize(size) +
      4 + frame->length();
}
