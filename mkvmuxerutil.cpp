...
bool WriteEbmlElement(IMkvWriter* writer, uint64 type, const char* value) {
  ...
  if (writer->Write(value, length)) return false;
...
uint64 WriteSimpleBlock(IMkvWriter* writer, const uint8* data, uint64 length,
  ...
  if (track_number < 1 || track_number > 126) return false;
  ...
  if (timecode < 0 || timecode > kMaxBlockTimecode) return false;

  if (WriteID(writer, kMkvSimpleBlock)) return 0;
  ...
  uint64 flags = 0;
  if (is_key) flags |= 0x80;

  if (SerializeInt(writer, flags, 1)) return 0;

  if (writer->Write(data, static_cast<uint32>(length))) return 0;
...
uint64 WriteMetadataBlock(IMkvWriter* writer, const uint8* data, uint64 length,
  ...
  if (SerializeInt(writer, duration, duration_payload_size)) return 0;
...
uint64 WriteBlockWithAdditional(IMkvWriter* writer, const uint8* data,
                                ...
                                uint64 track_number, int64 timecode,
                                uint64 is_key) {
  if (!data || !additional || length < 1 || additional_length < 1) return 0;
  ...
  if (!WriteEbmlMasterElement(writer, kMkvBlock, block_payload_size)) return 0;
  ...
  if (!WriteEbmlElement(writer, kMkvBlockAddID, add_id)) return 0;
...
uint64 WriteBlockWithDiscardPadding(IMkvWriter* writer, const uint8* data,
                                    ...
                                    uint64 track_number, int64 timecode,
                                    uint64 is_key) {
  if (!data || length < 1 || discard_padding <= 0) return 0;
