...
long long mkvparser::UnserializeUInt(IMkvReader* pReader, long long pos,
  ...
  if ((size <= 0) || (size > 8)) return E_FILE_FORMAT_INVALID;
...
long mkvparser::UnserializeFloat(IMkvReader* pReader, long long pos,
  ...
  if ((size_ != 4) && (size_ != 8)) return E_FILE_FORMAT_INVALID;
...
long mkvparser::ParseElementHeader(IMkvReader* pReader, long long& pos,
  ...
  if ((stop >= 0) && ((pos + size) > stop)) return E_FILE_FORMAT_INVALID;
...
bool mkvparser::Match(IMkvReader* pReader, long long& pos, unsigned long id_,
  ...
  if ((unsigned long)id != id_) return false;
...
long long EBMLHeader::Parse(IMkvReader* pReader, long long& pos) {
  ...
  for (;;) {
    ...
    if ((total >= 0) && ((total - pos) < 5)) return E_FILE_FORMAT_INVALID;

    if ((available - pos) < 5) return pos + 5;  // try again later
...
long long Segment::CreateInstance(IMkvReader* pReader, long long pos,
  ...
  for (;;) {
    ...
    if (id == 0x08538067)  // Segment ID
    {
      ...
      if (pSegment == 0) return -1;  // generic error
...
long Segment::DoLoadCluster(long long& pos, long& len) {
  ...
  if (status == 0)  // no entries found
  {
    if (cluster_size < 0) return E_FILE_FORMAT_INVALID;  // TODO: handle this
...
bool Cues::LoadCuePoint() const {
  ...
  while (m_pos < stop) {
    ...
    if (pCP->GetTimeCode() < 0 && (-pCP->GetTimeCode() != idpos)) return false;
...
#else
  if (m_cue_points == NULL) return false;
...
const CuePoint* Cues::GetFirst() const {
  if (m_cue_points == NULL) return NULL;
...
long Tracks::ParseTrackEntry(long long track_start, long long track_size,
  ...
  if (info.type == Track::kVideo) {
    ...
    if (e.start >= 0) return E_FILE_FORMAT_INVALID;
...
long Cluster::Load(long long& pos, long& len) const {
...

  {
    ...
    if (size == 0) return E_FILE_FORMAT_INVALID;  // TODO: verify this
  ...
  for (;;) {
    ...
    if (cluster_stop >= 0) {
      if (block_stop > cluster_stop) {
        if ((id == 0x20) || (id == 0x23)) return E_FILE_FORMAT_INVALID;
