...
long long BadReadID(IMkvReader* pReader, long long pos, long& len) {
...
long long EBMLHeader::Parse(IMkvReader* pReader, long long& pos) {
  ...
  while (pos < end) {
    ...
    if (id == mkvmuxer::kMkvEBMLVersion) {
      ...
      if (m_maxIdLength <= 0)
        return E_FILE_FORMAT_INVALID;
    } else if (id == mkvmuxer::kMkvEBMLMaxSizeLength) {  // max size length
...
long long Segment::ParseHeaders() {
  ...
  for (;;) {
    ...
    } else if (id == mkvmuxer::kMkvTags) {  // Tags ID
      if (m_pTags == NULL) {
...
bool SeekHead::ParseEntry(IMkvReader* pReader, long long start, long long size_,
  ...
  if (seekIdId != mkvmuxer::kMkvSeekID)  // SeekID ID
    return false;
...
long Track::ParseContentEncodingsEntry(long long start, long long size) {
  ...
  while (pos < stop) {
    ...
    // pos now designates start of element
    if (id == mkvmuxer::kMkvContentEncoding) {  // ContentEncoding ID
...
long Cluster::CreateBlockGroup(long long start_offset, long long size,
  ...
  while (pos < stop) {
    ...
    const long long size = ReadID(pReader, pos, len);
    assert(size >= 0);  // TODO
    assert((pos + len) <= stop);
