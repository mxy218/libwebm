...
long ContentEncoding::ParseContentEncAESSettingsEntry(
  ...
  while (pos < stop) {
    ...
    if (id == 0x7E8) {
      ...
      aes->cipher_mode =
           UnserializeUInt(pReader, pos, size);
      if (aes->cipher_mode != 1)
        return E_FILE_FORMAT_INVALID;
    } else if (id == 0x7E9) {
      // AESSettingsCipherInitData
      delete[] aes->cipher_init_data;
      aes->cipher_init_data = NULL;
      aes->cipher_init_data_len = 0;
      ...
      aes->cipher_init_data = buf;
      aes->cipher_init_data_len = buflen;
    }
