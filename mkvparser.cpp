...
long mkvparser::UnserializeString(IMkvReader* pReader, long long pos,
  ...
  const long status = pReader->Read(pos, required_size, buf);
...
long mkvparser::ParseElementHeader(IMkvReader* pReader, long long& pos,
  ...
  if (size < 0 || len < 0 || len > 8) {
    // Disallow negative payload size.
    // Disallow negative length integer.
    // Disallow integer larger than 64 bits (libwebm cannot handle them).
    return E_FILE_FORMAT_INVALID;
  }
  ...
  if ((pos + size) < 0 || ((stop >= 0) && ((pos + size) > stop)))
    return E_FILE_FORMAT_INVALID;
...
bool mkvparser::Match(IMkvReader* pReader, long long& pos,
  ...
  if (id < 0 || len < 1 || len > 8 || (pos + len) > available)
    return false;
...
bool mkvparser::Match(IMkvReader* pReader, long long& pos,
  ...
  assert(len <= 8);
  assert((pos + len) <= available);
  if (size < 0 || len < 1 || len > 8 || (pos + len) > available)
    return false;
