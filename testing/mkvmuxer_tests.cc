...
TEST_F(MuxerTest, CuesBeforeClusters) {
  ...
  ASSERT_TRUE(test::HasCuePoints(segment, &cues_offset));
  ASSERT_NE(0, cues_offset);
...
TEST_F(MuxerTest, Projection) {
  ...
  muxer_proj.type = mkvmuxer::Projection::kRectangular;
  muxer_proj.pose_yaw = 1;
  muxer_proj.pose_pitch = 2;
  muxer_proj.pose_roll = 3;
  ...
  EXPECT_FLOAT_EQ(parser_proj->pose_yaw, muxer_proj.pose_yaw);
  EXPECT_FLOAT_EQ(parser_proj->pose_pitch, muxer_proj.pose_pitch);
  EXPECT_FLOAT_EQ(parser_proj->pose_roll, muxer_proj.pose_roll);
  ASSERT_TRUE(parser_proj->private_data != nullptr);
  EXPECT_EQ(parser_proj->private_data_length,
            static_cast<size_t>(muxer_proj.private_data_length()));
  EXPECT_EQ(parser_proj->private_data[0], muxer_proj.private_data()[0]);
  typedef mkvparser::Projection::ProjectionType ParserProjType;
  EXPECT_EQ(parser_proj->type, static_cast<ParserProjType>(muxer_proj.type));
