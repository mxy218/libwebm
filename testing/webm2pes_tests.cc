...
class Webm2PesTests : public ::testing::Test {
 public:
  ...
  // Parses a validates a BCMV header.
  void ParseBcmvHeader(BcmvHeader* header) {
    ...
    header->length |= (pes_file_data_[offset++] & 0xff) << 24;
    header->length |= (pes_file_data_[offset++] & 0xff) << 16;
    header->length |= (pes_file_data_[offset++] & 0xff) << 8;
    header->length |= pes_file_data_[offset++] & 0xff;
...
TEST_F(Webm2PesTests, CanParseFirstPacket) {
  ...
  EXPECT_EQ(kPesOptionalHeaderMarkerValue, poh.marker);
  EXPECT_EQ(0, poh.scrambling);
  EXPECT_EQ(0, poh.priority);
  EXPECT_EQ(0, poh.data_alignment);
  EXPECT_EQ(0, poh.copyright);
  EXPECT_EQ(0, poh.original);
  EXPECT_EQ(1, poh.has_pts);
  EXPECT_EQ(0, poh.has_dts);
  EXPECT_EQ(0, poh.unused_fields);
  ...
  BcmvHeader bh;
  ParseBcmvHeader(&bh);
  EXPECT_EQ(kFirstFrameLength, bh.length);
