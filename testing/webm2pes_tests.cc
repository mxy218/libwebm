...
class Webm2PesTests : public ::testing::Test {
 public:
  ...
  bool VerifyPacketStartCode() {
    ...
    if (pes_file_data_[packet_start_pos_] != 0 ||
        pes_file_data_[packet_start_pos_ + 1] != 0 ||
        pes_file_data_[packet_start_pos_ + 2] != 1) {
      return false;
    }
    return true;
  }

  std::uint8_t ReadStreamId() { return pes_file_data_[packet_start_pos_ + 3]; }

  std::uint16_t ReadPacketLength() {
    // Read and byte swap 16 bit big endian length.
    return ((pes_file_data_[packet_start_pos_ + 4] & 0xff) << 8) |
           pes_file_data_[packet_start_pos_ + 5];
  }

  void ParsePesOptionalHeader(PesOptionalHeader* header) {
    EXPECT_TRUE(header != nullptr);
    std::size_t offset = packet_start_pos_ + kPesOptionalHeaderStartOffset;
...
TEST_F(Webm2PesTests, CanParseFirstPacket) {
  ...
  EXPECT_EQ(ReadPacketLength(), kPesPayloadLength);
