...
void vttdemux::BuildMap(
  ...
  if (segment->GetChapters()) {
    ...
    m[0] = info;  // no possibility of conflict using key 0
  }
...
bool vttdemux::WriteChaptersFile(
  ...
  if (chapters == NULL)  // no chapters, so nothing to do
    return true;

  const metadata_map_t::const_iterator info_iter = m.find(0);
  ...
  if (edition_count > 1) {
    ...
    printf("more than 1 chapter edition detected\n");
    return false;
  }
...
bool vttdemux::WriteChaptersCueTimings(
    ...
    const mkvparser::Chapters* chapters,
    const mkvparser::Chapters::Atom* atom) {
  const mkvparser::Segment* const segment = chapters->m_pSegment;
  ...
  const mkvtime_t start_timecode = atom->GetStartTimecode();

  if (start_timecode < 0)
    return false;

  const mkvtime_t start_ns = start_timecode * timecode_scale;
  ...
  const mkvtime_t stop_timecode = atom->GetStopTimecode();

  if (stop_timecode < 0)
    return false;

  const mkvtime_t stop_ns = stop_timecode * timecode_scale;
