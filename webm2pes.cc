...
bool PesOptionalHeader::Write(bool write_pts, PacketDataBuffer* buffer) const {
  ...
  *++byte = remaining_size.bits;  // Field is 8 bits wide.
  ...
  for (int i = 0; i < static_cast<int>(kHeaderSize); ++i)
...
bool BCMVHeader::Write(PacketDataBuffer* buffer) const {
  ...
  for (int i = 0; i < static_cast<int>(kBcmvSize); ++i)
  ...
  const uint8_t bcmv_buffer[kRemainingBytes] = {
      ...
      0,
      0 /* 2 bytes 0 padding */};
  for (int i = 0; i < static_cast<int>(kRemainingBytes); ++i)
...
bool PesHeader::Write(bool write_pts, PacketDataBuffer* buffer) const {
  ...
  for (int i = 0; i < static_cast<int>(kStartCodeLength); ++i)
