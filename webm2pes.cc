...
// Does no data validation; storage of a value too large for the given number
// of bits will fail silently. Maximum size is 64 bits.
...
struct PesOptionalHeader {
  ...
  // code writing the optionoal header to iterate over the fields instead of
  ...
  std::size_t size_in_bytes() const { return 9; }
};
...
struct BCMVHeader {
  ...
  bool Write(std::FILE* fileptr) {
    if (fileptr == nullptr) {
      printf("Webm2Pes: nullptr for file in BCMV Write.\n");
      return false;
    }
    if (std::fwrite(bcmv, 1, 4, fileptr) != 4) {
      printf("Webm2Pes: BCMV write failed.\n");
    }
    ...
    for (std::size_t i = 0; i < kRemainingBytes; ++i) {
      if (WriteUint8(buffer[i], fileptr) != true) {
        printf("Webm2Pes: BCMV remainder write failed.\n");
        return false;
      }
    }
    return true;
...
class Webm2Pes {
 ...
 private:
  ...
  struct FILEDeleter {
    int operator()(FILE* f) { return fclose(f); }
  };
...
bool Webm2Pes::Convert() {
  ...
  if (Segment::CreateInstance(&webm_reader_, 0 /* pos */,
    ...
    printf("Webm2pes: Cannot create WebM parser.\n");
    return false;
  }
  webm_parser_.reset(webm_parser);

  if (webm_parser_->Load() != 0) {
    printf("Webm2pes: Cannot parse %s.\n", input_file_name_.c_str());
    return false;
  }
  ...
  if (tracks == nullptr) {
    printf("Webm2pes: %s has no tracks.\n", input_file_name_.c_str());
    return false;
  }
  ...
  if (video_track_num_ < 1) {
    printf("Webm2pes: No video track found in %s.\n", input_file_name_.c_str());
    return false;
  }
  ...
  while (cluster != nullptr && cluster->EOS() == false) {
    ...
    if (block_status < 0) {
      printf("Webm2pes: Cannot parse first block in %s.\n",
             input_file_name_.c_str());
      return false;
    }
    ...
    while (block_entry != nullptr && block_entry->EOS() == false) {
      ...
      if (block->GetTrackNumber() == video_track_num_) {
        ...
        for (int frame_num = 0; frame_num < frame_count; ++frame_num) {
          ...
          if (pes_status != true) {
            printf("Webm2pes: WritePesPacket failed.\n");
            return false;
          }
        }
      }
      ...
      if (block_status < 0) {
        printf("Webm2pes: Cannot parse block in %s.\n",
               input_file_name_.c_str());
        return false;
      }
    }
...
bool Webm2Pes::Write90KHzPtsBitsToPesHeader(std::int64_t pts_90khz,
                                            PesHeader* header) const {
  if (header == nullptr) {
    printf("Webm2Pes: cannot write PTS to nullptr.\n");
    return false;
  }
...
bool Webm2Pes::WritePesOptionalHeader(const PesHeader& pes_header,
                                      ...
                                      std::uint8_t* header) const {
  if (header == nullptr) {
    printf("Webm2Pes: nullptr in opt header writer.\n");
    return false;
  }
  ...
  *byte = poh.stuffing_byte.bits;
  ...
  if (std::fwrite(reinterpret_cast<void*>(header), 1, poh.size_in_bytes(),
    ...
    printf("Webm2Pes: unable to write PES opt header to file.\n");
    return false;
  }
...
bool Webm2Pes::WritePesPacket(const mkvparser::Block::Frame& vpx_frame,
  ...
  if (packetize == false) {
    ...
    if (std::fwrite(reinterpret_cast<const void*>(header.start_code), 1, 4,
                    output_file_.get()) != 4) {
      printf("Webm2Pes: cannot write packet start code.\n");
      return false;
    }

    // Big endian length.
    std::uint8_t byte = (header.packet_length & 0xff00) >> 8;
    if (WriteUint8(byte, output_file_.get()) != true) {
      printf("Webm2Pes: cannot write packet length (1).\n");
      return false;
    }
    byte = header.packet_length & 0xff;
    if (WriteUint8(byte, output_file_.get()) != true) {
      printf("Webm2Pes: cannot write packet length (2).\n");
      return false;
    }

    if (WritePesOptionalHeader(header, header.packet_length,
                               &write_buffer[0]) != true) {
      printf("Webm2Pes: PES optional header write failed.");
      return false;
    }
    ...
    if (frame_data.get() == nullptr) {
      printf("Webm2Pes: Out of memory.\n");
      return false;
    }
    if (vpx_frame.Read(&webm_reader_, frame_data.get()) != 0) {
      printf("Webm2Pes: Error reading VPx frame!\n");
      return false;
    }
    if (std::fwrite(frame_data.get(), 1, vpx_frame.len, output_file_.get()) !=
        vpx_frame.len) {
      printf("Webm2Pes: VPx frame write failed.\n");
      return false;
    }

  } else {
    printf("Webm2pes: Packetization of large frames not implemented yet.\n");
    return false;
  }
