...
struct PacketInfo {
  PacketInfo(bool keyframe, std::int64_t packet_timestamp)
      : is_keyframe(keyframe), timestamp(packet_timestamp) {}
  ~PacketInfo() = default;
  PacketInfo() = delete;
  ...
  // Timestamp for the frame, in nanoseconds.
...
class PacketReceiverInterface {
 public:
  ...
  virtual bool ReceivePacket(const PacketInfo& packet_info,
