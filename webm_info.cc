...
// Prints the partition offsets of the sub-sample encryption. Returns the
// number of bytes read from the sub-sample partition information.
int PrintSubSampleEncryption(const uint8_t* data, int size, FILE* o) {
  ...
  for (int i = 0; i < num_partitions; ++i) {
    ...
    fprintf(o, " off[%d]:%d", i, partition_offset);
      ...
      if ((track_type == mkvparser::Track::kVideo && options.output_video) ||
        ...
        if (options.output_encrypted_info) {
          ...
          if (encrypted_stream) {
            ...
            const bool sub_sample_encrypt = (data[0] & kSubSampleBit) ? 1 : 0;
