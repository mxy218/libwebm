...
void PrintVP9Info(const uint8_t* data, int size, FILE* o, int64_t time_ns,
  ...
  do {
    size_t frame_length = size;
    if (count > 0)
      frame_length = sizes[i];
...
int main(int argc, char* argv[]) {
  ...
  if (options.output_vp9_level) {
    level_stats.SetDuration(segment->GetInfo()->GetDuration());
    vp9_parser::Vp9Level level = level_stats.GetLevel();
    fprintf(out, "VP9 Level:%d\n", level);
    fprintf(out, "mlsr:%" PRId64 " mlps:%" PRId64 " abr:%g mcs:%g cr:%g mct:%d"
            " mad:%d mrf:%d\n",
