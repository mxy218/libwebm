...
int Parser::ParseTime(
  ...
  // Here we slightly relax the syntax rules for WebVTT timestamps, to
  // specifically allow the comma character to also be used as the
  // seconds/milliseconds separator.  This will handle streams that
  // use localization conventions for countries in Western Europe.

  const bool have_milliseconds = (c == '.' || c == ',');

  if (!have_milliseconds) {
    t.milliseconds = 0;
  } else {
    ++idx;  // consume FULL STOP or COMMA
